...
#' @description Creates similar articles graph and corresponding data.frame with
...
#' @param n_cites_threshold An integer. The minimum number of papers that a given article should be similar to.
#'
#' @return A data.frame with info about papers that are similar to a given \code{PMID_list}.
#' @export
graph_foo_similar <- function(PMID_list,
                               ...
                               year_right = substr(Sys.Date(), 1, 4))
{
  ...
  data_table <- similar_articles(PMID_list, year_left, year_right)
  ...
  deg_to_csv <- data.frame(PMID=names(deg0), n_similar=deg0, row.names=NULL) %>%
    arrange(desc(n_similar))


  # Фильруем по минимальному количеству статей, на которые должна быть похожа заданная статья
  # Получаем список отобранных PMID для дальнейшего прогона
  double_info_PMID_list <- deg_to_csv %>%
    dplyr::filter(n_similar >= n_similar_threshold) %>%
    pull(PMID)
  ...
  top_papers_double_to_csv <- merge(top_papers_double, deg_to_csv, by = "PMID")  %>%
    replace(is.na(.), 0)  %>%
    arrange(desc(n_similar), desc(n_cited))
